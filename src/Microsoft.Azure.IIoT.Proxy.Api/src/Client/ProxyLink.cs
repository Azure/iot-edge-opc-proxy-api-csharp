...
namespace Microsoft.Azure.IIoT.Proxy.Client {
    ...
    internal class ProxyLink : IProxyLink {
        ...
        public async Task SetSocketOptionAsync(
            ...
            using (var request = Message.Create(_socket.LocalId, RemoteId, SetOptRequest.Create(
        ...
        public async Task<ulong> GetSocketOptionAsync(
            ...
            using (var request = Message.Create(_socket.LocalId, RemoteId, GetOptRequest.Create(
                option))) {
        ...
        private async Task TerminateConnectionAsync(CancellationToken ct) {
            ...
            await Try.Async(() => SendBlock.SendAsync(Message.Create(_socket.LocalId, 
                RemoteId, CloseRequest.Create()), ct)).ConfigureAwait(false);
            await Try.Async(connection.CloseAsync).ConfigureAwait(false);
