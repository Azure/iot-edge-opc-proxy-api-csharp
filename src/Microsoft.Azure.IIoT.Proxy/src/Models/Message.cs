...
namespace Microsoft.Azure.IIoT.Proxy.Models {
    ...
    public class Message : Poco<Message> {
        ...
        /// </summary>
        public static Message Create(Reference source, Reference target,
            Reference proxy, IMessageContent content, int error = 0) =>
            Create((uint)Interlocked.Increment(ref _counter), source, target, proxy, content, error);

        /// <summary>
        /// Create response
        /// </summary>
        public static Message Create(Message message, IMessageContent content) =>
            Create(message.Target, message.Source, message.Proxy, content);

        /// <summary>
        /// Create response message
        /// </summary>
        public static Message Create(Message message, IMessageContent content, int error) =>
            Create(message.Target, message.Source, message.Proxy, content, error);

        /// <summary>
        /// Create response message
        /// </summary>
        public static Message Create(Message message, IMessageContent content, Exception error) =>
            Create(message.Target, message.Source, message.Proxy, content,
                (int)error.GetProxySocketError());

        /// <summary>
        /// Create message with specific buffer
        /// </summary>
        protected static Message Create(uint sequenceId, Reference source,
            Reference target, Reference proxy, IMessageContent content, int error,
        ...
        /// </summary>
        public Message Clone() => Create(SequenceId, Source, Target, Proxy, Content?.Clone(), Error, DeviceId);
